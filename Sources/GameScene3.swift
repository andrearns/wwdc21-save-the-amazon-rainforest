...
public class GameScene3: SKScene, SKPhysicsContactDelegate {
    ...
    // Animals
    var oncaPintada: SKSpriteNode!
    
    // Pop ups
    var oncaPintadaPopUp: SKSpriteNode!
    var nextButtonOncaPintada: SKSpriteNode!
    ...
    // Big Fire
    var bigFire: SKSpriteNode!
    var xBigFireSpeed: CGFloat = 0
...
//        view.showsPhysics = true
        ...
        oncaPintada = self.childNode(withName: "OncaPintada") as? SKSpriteNode
        oncaPintada.texture = SKTexture(imageNamed: "oncapintada-jaula")
        ...
        // Add the big fire on the scene
        bigFire = self.childNode(withName: "BigFire") as? SKSpriteNode
        setupBigFireAnimation()
        bigFire.run(bigFireAnimation)
        ...
        oncaPintadaPopUp = SKSpriteNode(imageNamed: "oncapintada-popup")
        
        oncaPintadaPopUp.setScale(0.75)
        oncaPintadaPopUp.alpha = 0
        oncaPintadaPopUp.zPosition = 1000
        oncaPintadaPopUp.position = CGPoint(x: -45, y: 50)
        
        nextButtonOncaPintada = SKSpriteNode(imageNamed: "oncapintada-nextbutton")
        
        nextButtonOncaPintada.setScale(1)
        nextButtonOncaPintada.alpha = 0
        nextButtonOncaPintada.zPosition = 1000
        nextButtonOncaPintada.position = CGPoint(x: -45, y: -200)
        
        overlay = SKSpriteNode(color: UIColor.black, size: self.size)
        overlay.alpha = 0
        
        cam.addChild(overlay)
        cam.addChild(oncaPintadaPopUp)
        cam.addChild(nextButtonOncaPintada)
        ...
        // Talk
        conversation = SKSpriteNode(imageNamed: "curupiratalks2")
    ...
    func releaseAnimal() {
        ...
        oncaPintada.texture = SKTexture(imageNamed: "oncapintada-livre")
        oncaPintada.yScale = 0.65
        oncaPintada.zPosition = 999
    ...
    func popUp() {
        ...
        oncaPintadaPopUp.run(SKAction.sequence([delay, fadeInPopUp]))
        overlay.run(SKAction.sequence([delay, fadeInOverlay]))
        nextButtonOncaPintada.run(SKAction.sequence([delay, fadeInPopUp]))
    }
    ...
    func talk() {
        let talkDuration = SKAction.wait(forDuration: 2)
    ...
    public func didBegin(_ contact: SKPhysicsContact) {
        ...
        if contact.bodyA.node?.name == "BigFire" || contact.bodyB.node?.name == "BigFire" {
            hurt()
        }
        else if contact.bodyA.node?.name == "BearTrap1" || contact.bodyB.node?.name == "BearTrap1" {
            run(bearTrapSound)
            hurt()
    ...
    func touchDown(atPoint pos : CGPoint) {
        ...
        else if gameStatus == .finish && nextButtonOncaPintada.contains(convert(pos, to: cam)) && instructionsStatus == .off {
            if let scene = JungleFire2(fileNamed: "JungleFire2") {
    ...
    override public func update(_ currentTime: TimeInterval) {
        ...
        bigFire.position.x += xBigFireSpeed * CGFloat(deltaTime)
        ...
        if bigFire.position.x > 7400 {
            gameOver()
            print("Big fire hit the animal")
        }
        
        if gameStatus == .finish {
            let moveCameraForPopUp = SKAction.move(to: CGPoint(x: oncaPintada.position.x - 400, y: oncaPintada.position.y + 200) , duration: 1)
